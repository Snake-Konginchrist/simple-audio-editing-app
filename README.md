...
- **单文件模式**：将所有依赖和资源打包为单个可执行文件，方便分发，但体积较大。
- **文件夹模式**：生成一个包含多个文件的文件夹，体积较小，但分发时需要保持文件结构完整。文件夹模式还会生成日志文件（位于logs目录），便于调试和问题排查。
...
6. **切换语言**：
   - 点击"文件"菜单。
   - 在下拉菜单中选择"语言"。
   - 从子菜单中选择所需语言（简体中文或English）。
   - 程序将询问是否立即重启应用以应用新的语言设置。
